...
use Evoweb\Sessionplaner\Updates\RowUpdater\RowUpdaterInterface;
...
class UpgradeWizardsService
{
    ...
    public function getWizardUserInput(string $identifier): array
    {
        ...
        if (method_exists($updateObject, 'getUserInput')) {
            ...
            $markup[] = '        <p>' . nl2br(htmlspecialchars($updateObject->getConfirmation()->getMessage()))
                . '</p>';
            $markup[] = '        <div class="btn-group" data-toggle="buttons">';
            if (!$updateObject->getConfirmation()->isRequired()) {
                $markup[] = '        <label class="btn btn-default active"><input '
                    . GeneralUtility::implodeAttributes($radioAttributes, true)
                    . ' checked="checked" />' . $updateObject->getConfirmation()->getDeny() . '</label>';
            }
            $radioAttributes['value'] = 1;
            $markup[] = '            <label class="btn btn-default"><input '
                . GeneralUtility::implodeAttributes($radioAttributes, true)
                . ' />' . $updateObject->getConfirmation()->getConfirm() . '</label>';
            $markup[] = '        </div>';
            $markup[] = '    </div>';
            $markup[] = '</div>';
    ...
    public function executeWizard(string $identifier): FlashMessageQueue
    {
        ...
        if (
            method_exists($updateObject, 'checkUserInput') &&
            ...
            if ($updateObject instanceof UpgradeWizardInterface) {
                ...
                if ($updateObject instanceof ConfirmableInterface) {
                    ...
                    if ($checkValue === 1) {
                        ...
                        // confirmation = no, but it is *not* required, we do *not* the update, but mark wizard as done
            ...
            if ($performResult) {
                if (
                    $updateObject instanceof UpgradeWizardInterface
    ...
    public function markWizardAsDone(string $identifier): void
    {
        ...
        /** @var Registry $registry */
        $registry = GeneralUtility::makeInstance(Registry::class);
        $registry->set('installUpdate', $class, 1);
    }
    ...
    public function isWizardDone(string $identifier): bool
    {
        ...
        /** @var Registry $registry */
        $registry = GeneralUtility::makeInstance(Registry::class);
        return (bool)$registry->get('installUpdate', $class, false);
    }
